...
<%   
        ...
        if(kgString==null){kgString=""+0;}
        if(country==null){country="Denmark";}
        ...
        Double fedexE, fedexP,upsSa,upsE, upsSt,glsS = 0.0;
            ...
            weight = c.weights[kg];
            if(fedexEconomy.length>1){fedexE = fedexEconomy[kg];} else {fedexE = 0.0;}
            if(fedexPriority.length>1){fedexP = fedexPriority[kg];} else {fedexP = 0.0;}
            if(upsSaver.length>1){upsSa = upsSaver[kg];} else {upsSa = 0.0;}
            if(upsExpress.length>1){upsE = upsExpress[kg];} else {upsE = 0.0;}
            if(upsStandard.length>1){upsSt = upsStandard[kg];} else {upsSt = 0.0;}
            if(gls.length>1 && kg<100){glsS = gls[kg];} else {glsS = 0.0;}
